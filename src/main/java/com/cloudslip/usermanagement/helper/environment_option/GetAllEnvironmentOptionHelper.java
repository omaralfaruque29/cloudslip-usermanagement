...
@Service
public class GetAllEnvironmentOptionHelper extends AbstractHelper {
    ...
    protected void doPerform() {
        if(input.getFetchMode() == null || input.getFetchMode().equals(ListFetchMode.PAGINATION)) {
            output.generateSuccessResponse(environmentOptionRepository.findAllByStatus(pageable, Status.V));
        } else if(input.getFetchMode() != null || input.getFetchMode().equals(ListFetchMode.ALL)) {
            output.generateSuccessResponse(environmentOptionRepository.findAllByStatus(Status.V));
        }
     }
